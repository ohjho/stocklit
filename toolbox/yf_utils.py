...
def valid_stock(stock_obj):
    ...
    """
    try:
        return "longName" in stock_obj.info.keys()
        ...
        # return 'symbol' in stock_obj.fast_info.keys()
    except:
        return False
...
def tickers_parser(tickers, return_list=False, max_items=None):
    ...
    if tickers:
        ...
        l_tickers = [
            ticker.split(".")[0].zfill(4) + ".HK" if ".HK" in ticker.upper() else ticker
...
def get_stocks_data(
    ...
    if yf_download_params["interval"] == "1wk":
        print(
            f"converting daily prices to weekly with internal method df_to_weekly() for {tickers}"
        )
...
def get_stocks_ohlc(
    tickers, session=SESH, interval="1d", start_date=None, end_date=None, proxies=None
):
    ...
    session = (
        update_session_proxy(session, proxy=random.choice(proxies))
        if proxies
        else session
    )
    prices_df = yf.download(
        tickers=tickers,  # session = session,
        period="max",
        interval="1d",
        group_by="ticker",
        progress=False,
    ...
    prices_df = (
        prices_df[prices_df.index > pd.Timestamp(start_date)]
        if start_date
        else prices_df
    )
    prices_df = (
        prices_df[prices_df.index < pd.Timestamp(end_date)] if end_date else prices_df
    )
    # Interval Adjustment
...
def get_dfs_by_tickers(df):
    ...
    ), "this function only works with DF with MultiIndex Columns"
...
def get_stock_info(
    ...
    assert valid_stock(
        yf.Ticker(ticker)
    ), f"{ticker} does not seem to be a valid stock ticker"
...
def get_stock_financials(ticker, key, b_transpose=True):
    ...
    financials_func_dict = {
        "balance_sheet": yq_ticker.balance_sheet,
        "income_statement": yq_ticker.income_statement,
        "cashflow": yq_ticker.cash_flow,
    }
    assert key in list(financials_func_dict.keys()) + [
        "earnings"
    ], f"unknown key: {key}"
    if key == "earnings":
...
def df_aggregate(
    ...
    assert rule in [
        "M",
        "W",
    ], f"df_aggregate: only weekly, W, and monthly, M offset available for now"
    if len(df_daily) > 0:
        ...
        df.index -= to_offset("6D") if rule == "W" else pd.offsets.MonthBegin(1)
        return df
    else:
        return df_daily
