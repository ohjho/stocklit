...
def show_plotly(fig, height=None, title=None, template="plotly_dark", st_asset=st):
    ...
    st_asset.plotly_chart(fig, use_container_width=True, height=height, theme=None)
...
def plotly_hist_draw_hline(fig, l_value_format):
    ...
    for shape in l_value_format:
        shape_dict = {
            ...
            "line": (
                shape["line_format"]
                if "line_format" in shape.keys()
                else default_line_format
            ),
        }
    ...
    fig.update_layout(shapes=l_shapes)


def get_timeframe_params(
    st_asset,
    data_buffer_tenor="1y",
    default_tenor="250b",
    l_interval_options=[
        "1d",
        "1m",
...
def get_json_edit(
    in_json,
    str_msg="Please edit your JSON object",
    text_area_height=500,
    json_dumps_kargs={"indent": 4, "sort_keys": True},
):
    out_json = st.text_area(
        str_msg, height=text_area_height, value=json.dumps(in_json, **json_dumps_kargs)
    )
    return json.loads(out_json) if is_json(out_json) else None


def get_sp_data(
    form_key, data_model, st_asset, submit_label="Submit", do_cache=False, **kwargs
):
    ...
    if do_cache:
        ...
        with st_asset.form(key=form_key):
            ...
            submitted = st.form_submit_button(
                label=submit_label if submit_label else "Submit"
            )
        if submitted:
            data = input_data
            ...
            data = (
                st.session_state[cache_key] if cache_key in st.session_state else None
            )
    else:
        with st_asset:
            data = sp.pydantic_form(
                key=form_key, model=data_model, submit_label=submit_label, **kwargs
            )
    return data
