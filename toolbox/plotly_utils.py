...
def add_channel_trace(fig, df, ch_cols, date_col=None, rgb_tup=(255, 222, 0)):
    ...
    """
    l_ch = [
        float(col.split("+")[-1].replace("atr", "")) for col in ch_cols if "+" in col
    ]
...
def add_color_event_ohlc(
    ...
    fig.add_trace(
        go.Candlestick(
            x=df_c[date_col] if date_col else df_c.index,
...
def add_volume_profile(
    fig, df, vol_col, price_col, show_legend=False, color="Ivory", opacity=0.6
):
...
def plotly_ohlc_chart(
    ...
    if vol_col:
        ...
        fig.add_trace(
            go.Bar(x=date_serie, y=df[vol_col], name="volume", showlegend=show_legend),
            row=2,
            col=1,
        )
    ...
    if len(ma_cols) > 0:
        for ma in ma_cols:
            fig.add_trace(go.Scatter(x=date_serie, y=df[ma], name=ma), row=1, col=1)
    ...
    for c in auto_subplot_col:
        if c in df.columns:
            ...
            if c == "RSI":
                fig.append_trace(
                    go.Scatter(x=date_serie, y=df["RSI"], name="RSI"),
                    row=ref_row,
                    col=1,
                )
                fig.add_hline(y=tup_rsi_hilo[1], line_dash="dot", row=ref_row, col=1)
                fig.add_hline(y=tup_rsi_hilo[0], line_dash="dot", row=ref_row, col=1)
            elif c == "ADX":
                fig = add_ADX_trace(fig, df, ref_row=ref_row, date_col=date_col)
            elif c == "MACD_histogram":
                fig = add_MACD_trace(
                    fig, df, ref_row=ref_row, date_col=date_col, draw_signal_line=False
                )
            elif c == "MACD":
                fig = add_MACD_trace(
                    fig, df, ref_row=ref_row, date_col=date_col, draw_signal_line=True
                )
            else:  # very simple scatter plot
                fig.append_trace(
                    go.Scatter(x=date_serie, y=df[c], name=c), row=ref_row, col=1
                )
